...
def tokenize(in_str):
    ...
    cs.reset()
    if not cs.legal():
        raise Exception("No string to tokenize.")
    while cs.uplegal():
        if "%" == cs.item:
...
class Char_stream(Stream):
    ...
    def scan_package_filenames(self):
        ...
        file = ""
        if not "{" == item:
            raise Exception("\\usepackage not followed by brace.")
        item = self.next()
        while self.uplegal() and not blank_or_rbrace_re.match(item):
            file += item
            item = self.next()
...
class Tex_stream(Stream):
    ...
    def scan_group(self):
        ...
        """
        if not self.legal():
            raise Exception("No group to scan.")
        item = self.item
    ...
    def scan_command_name(self):
        ...
        """
        if not self.legal():
            raise Exception("No command name to scan.")
        item = self.item
        name = ""
        if item.type in [esc_symb_ty, esc_str_ty]:
            ...
            if not "{" == item.val:
                raise Exception("Command definition misses first {.")
            self.next()
            item = self.skip_blank_tokens()
            if not item.type in [esc_symb_ty, esc_str_ty]:
                raise Exception("Command definition does not begin with control sequence.")
            name = item.val
            self.next()
            item = self.skip_blank_tokens()
            if not "}" == item.val:
                raise Exception(("Definition for commmand %s misses first }., %s" %
                       (name, item.val)))
        self.next()
    ...
    def scan_numargs(self, name):
        ...
        """
        if not self.legal():
            raise Exception("No numargs to scan.")
        item = self.item
        numargs = 0
        if not simple_ty == item.type:
            raise Exception("Illegal command or environment definition: "+name)
        if "[" == item.val:
            if not 4 < len(self.data):
                raise Exception("Command or environment definition is illegal: "+name)
            item = self.next()
            if not simple_ty == item.type:
                raise Exception("Illegal command or environment definition: "+name)
            numargs = item.val
            if not pos_digit_re.match(numargs):
                raise Exception("%s must be argument number after %s" % (numargs, name))
            numargs = int(numargs)
            self.next()
            item = self.skip_blank_tokens()
            if not simple_ty == item.type:
                raise Exception("Illegal command definition: "+name)
            if "]" != item.val:
                raise Exception("Illegal command definition: "+name)
            self.next()
            self.skip_blank_tokens()
    ...
    def add_defs(self, defs_file):
        ...
        if not os.path.isfile(defs_file_compl):
            raise Exception("%s does not exist" % (defs_file_compl))
    ...
    def apply_command_recur(self, command_instance):
        ...
        try:
            ...
            raise Exception("apply_all_recur fails on command instance %s: %s, %s" % \
    ...
    def process_file(self, file):
        ...
        if not self.data:
            raise Exception("Empty tokenization result.")
        self.reset()
